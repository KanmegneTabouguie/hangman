...
func main() {
	...
	go func() {
		...
		c := cors.New(cors.Options{
			AllowedOrigins: []string{"http://localhost:3003"}, // Add your React app's URL
		...
		http.Handle("/init", handler)
		http.Handle("/guess", handler)
		// Serve static files (HTML, CSS, JS)
		http.Handle("/", http.FileServer(http.Dir("hangman-frontend/build")))
